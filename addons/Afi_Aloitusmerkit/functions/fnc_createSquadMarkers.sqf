...
if (GVAR(showFriendlyMarkers) && {playerSide != civilian}) then {
	_groupsToCreateMarkers = allGroups select {[side _x, playerSide] call BIS_fnc_sideIsFriendly && {(side _x != civilian)}};
} else {
	_groupsToCreateMarkers = allGroups select {side _x == playerSide};
};
